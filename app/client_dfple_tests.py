...
class SecretsTestCase(DFPLEClientTestCase):
    ...
    def test_secret(self):
        ...
        mocked_data = {
            'service_dfp': docker.models.services.Service(
                attrs={'Spec': {'Name': 'proxy', 'TaskTemplate': {
                    'ContainerSpec': {'Image': '', 'Secrets': []}}, 'Networks': [], }}
            ),
        ...
        with patch('client_dfple.DFPLEClient.secrets', return_value=mocked_data['secrets_initial']), \
            ...
            dfp_secrets = self.client.service_get_secrets(
                self.client.services(self.client.dfp_service_name)[0])

            # check certs do not exist
            for d in self.domains:
                self.assertFalse(
                    any(['{}.pem'.format(d) in x for x in certs[d]]))

            # check secrets not found and not attached
            for d in self.domains:
                self.assertFalse(any([d in x.name for x in secrets]))
                self.assertFalse(any([d == x['SecretName']
                                      for x in dfp_secrets]))
            ...
            for d in self.domains:
                self.assertTrue(
                    any([d in x.name for x in self.client._secrets]))
                self.assertTrue(any([d == x['SecretName']
                                     for x in self.client.dfp_secrets]))
    ...
    def test_secret_not_created_not_attached(self):
        ...
        with patch('client_dfple.DFPLEClient.secrets', return_value=mocked_data['secrets_initial']), \
            ...
            self.letsencrypt_mock(self.domains, None, None, None, tmp_files=[
                                  'privkey.pem', 'fullchain.pem', 'combined.pem'])
            ...
            # check secrets found and attached
            for d in self.domains:
                self.assertTrue(any([d in x.name for x in self.client._secrets]), '{} not found in {}'.format(
                    d, [x.name for x in self.client._secrets]))
                self.assertTrue(any([d == x['SecretName']
                                     for x in self.client.dfp_secrets]))
    ...
    def test_secret_created_not_attached(self):
        ...
        with patch('client_dfple.DFPLEClient.secrets', return_value=mocked_data['secrets_initial']), \
            ...
            for d in self.domains:
                ...
                self.assertTrue(
                    any(['{}.pem'.format(d) == x['SecretName'] for x in self.client.dfp_secrets]))
